export type recommendation = {
  role: string;
  name: string;
  year: string;
  review: string;
  image: string;
  link: string;
};


export const recommendationData: recommendation[] = [
  {
    name: "Yufei Li",
    role: "Doctoral Student | Mechanical & Aerospace Engineering | Nanyang Technological University",
    year: "August 31, 2023",

    review: "I highly recommend Yujin for his exceptional expertise in Python programming, machine learning, \
              and data optimization. His unwavering commitment to excellence and eagerness to share his knowledge has left a \
              profound impact on my learning journey and the coding aspect of my PhD studies.\
              \n\n \
              During a particularly challenging section of my code, Yujin helped me with an impressive depth of \
              understanding in Python programming. He not only helped me identify and resolve complex issues in my \
              code but also patiently guided me through the debugging process, ensuring that I grasped the underlying concepts. \
              \n\n \
              What sets Yujin apart is his ability to break down complex concepts into understandable components, \
              making it easy for those around them to learn and grow. His dedication to collaboration and knowledge sharing \
              has not only strengthened my technical skills but also inspired me to tackle more ambitious projects for my \
              academic pursuit with confidence. \
              \n\n \
              I am genuinely grateful for Yujin's support, patience, and expertise. \
              His invaluable contributions to my PhD Studies have not only resolved technical roadblocks \
              but have also enriched my understanding of Python programming and machine learning. Without a doubt, \
              Yujin is an asset to any team and an individual whose commitment to excellence is truly commendable.",
    image: "https://media.licdn.com/dms/image/C5603AQE0QLUXijb--w/profile-displayphoto-shrink_100_100/0/1606118614865?e=1698883200&v=beta&t=KLUBvw0twTMJiYh1RiH1y3IwgQIDgKgxrbgM2HEEbSo",
    link: "https://www.linkedin.com/in/yu-jin-goh-737939143/details/recommendations/",
  },
  {
    name: "Edwin Myloth",
    role: "IEC62443 | NSE4 | Regional Solution Architect at Fortinet",
    year: "August 23, 2018",
    review: "A highly talented individual who is always willing to offer his help to other students. \
              Yu Jin is well versed in various programming languages such as Java, python, C++, C, javascript etc. \
              Yu Jin has helped me in troubleshooting various programming issues throughout my Masters graduate course. \
              Yu Jin is also highly knowledgable about domains on Artificial intelligence and Cyber security \
              and the tools used in them. \
              Yu Jin is a pleasant individual to work with and I highly recommend him for future job prospects.",
    image: "https://media.licdn.com/dms/image/D5603AQEI0CfWFP_hyA/profile-displayphoto-shrink_100_100/0/1680244704807?e=1698278400&v=beta&t=HUd0Vd3QhB8ZiaxPB1YYi45sw-1-hhrFKL1cS8Xz8FM",
    link: "https://www.linkedin.com/in/yu-jin-goh-737939143/details/recommendations/",
  },
  {
    name: "Graeme Chiew",
    role: "CISSP | GCIH | ICS/OT Cybersecurity",
    year: "August 23, 2018",
    review: "Yu Jin is one of the most outstanding, talented individuals I have come across. \
              He's an extremely fast learner - having mastered java and python before commencing his studies at SUTD,\
              he served the role of a teaching assistant under our common mentor, where he taught \
              Python, Tableau, Java, WEKA and R to a class of Master students. He's one of the rare people you’d come\
              across that simply excels at every thing he does. \
              Someone you can definitely rely on - an asset to any team!",
    image: "https://media.licdn.com/dms/image/C4D03AQHPIluWwlCnJQ/profile-displayphoto-shrink_100_100/0/1655404958714?e=1698278400&v=beta&t=89aYc1zEfLeq2duyngMfCCzrAK7MRyPgGgDcPPSrdls",
    link: "https://www.linkedin.com/in/yu-jin-goh-737939143/details/recommendations/",
  },
];